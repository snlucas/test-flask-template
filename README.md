...
- 🎯 Entry points to execute your program using `python -m <test_flask_template>` or `$ test_flask_template` with basic CLI argument parsing.
...
---
# test_flask_template Flask Application

Awesome test_flask_template created by snlucas
...
```bash
git clone https://github.com/snlucas/test-flask-template test_flask_template
cd test_flask_template
make install
```
...
```bash
pip install test_flask_template
```
...
```bash
test_flask_template create-db   # run once
test_flask_template populate-db  # run once (optional)
test_flask_template add-user -u admin -p 1234  # ads a user
test_flask_template run
```

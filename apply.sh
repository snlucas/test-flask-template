...
original_author="snlucas"
original_name="test_flask_template"
original_urlname="test-flask-template"
original_description="Awesome test_flask_template created by snlucas" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/test_flask_template "${name}"
...
then
    ...
    echo "then run 'test_flask_template' or 'python -m test_flask_template'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'test_flask_template' or 'python -m test_flask_template' or 'poetry run test_flask_template'"
fi
